...
class MovieDetails(models.Model):
    ...
    header_id = models.ForeignKey(MovieHeader,on_delete=models.CASCADE,related_name="movie_details",db_column="header_id",blank=True, null=True)
    ...
    platform_id = models.ForeignKey(MoviePlatforms,on_delete=models.CASCADE,related_name='movie_details',db_column='platform_id',blank=True, null=True )
    status = models.CharField(max_length=50, choices=[(e.name, e.value) for e in EntityStatus], default=EntityStatus.PENDING.value)
    geners_id = models.ForeignKey(MovieGeners,on_delete=models.CASCADE,related_name="movie_details",db_column="geners_id",blank=True, null=True)
    user_id = models.ForeignKey(Register, on_delete=models.SET_NULL, related_name='movie_details', null=True, db_column='user_id')
    publish_at = models.DateTimeField(null=True, blank=True)  # Field to schedule when the details is published
    language = models.ForeignKey(Language,on_delete=models.CASCADE,related_name="movie_details",db_column="language",blank=True, null=True)
