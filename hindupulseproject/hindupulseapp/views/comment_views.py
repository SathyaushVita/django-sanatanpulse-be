...
from ..serializers import CommentSerializer, CommentSerializer1
...
class CommentView(viewsets.ModelViewSet):
    queryset = CommentModel.objects.filter(is_reply=IsReplyEnum.NO.value)  # Filter out replies
    ...
    def list(self, request, *args, **kwargs):
        queryset = CommentModel.objects.filter(is_reply=IsReplyEnum.NO.value).order_by('-created_at')
...
class GetCommentById_InputView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if articles:
            queryset = queryset.filter(articles=articles)
...
class LikeCommentView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            comment = CommentModel.objects.get(_id=comment_id)
