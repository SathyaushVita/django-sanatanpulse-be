...
from ..serializers import News_CommentSerializer1,News_CommentSerializer
...
class News_CommentView(viewsets.ModelViewSet):
    queryset = NewsCommentModel.objects.filter(is_reply=IsReplyEnum.NO.value)  # Filter out replies
    serializer_class = News_CommentSerializer1  # Default serializer class for create

    def get_serializer_class(self):
        if self.action == 'list':
            return News_CommentSerializer
        return self.serializer_class
...
class News_GetCommentById_InputView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = News_CommentSerializer(paginated_queryset, many=True)
        return paginator.get_paginated_response(serializer.data)
    
class News_LikeCommentView(APIView):
    def post(self, request, *args, **kwargs):
        comment_id = kwargs.get('comment_id')
        try:
            comment = NewsCommentModel.objects.get(_id=comment_id)
            if comment.likes is None:
                comment.likes = 0
            comment.likes += 1
            comment.save()
            return Response({"status": "comment liked"}, status=status.HTTP_200_OK)
        except NewsCommentModel.DoesNotExist:
            return Response({"error": "Comment not found"}, status=status.HTTP_404_NOT_FOUND)


class News_DislikeCommentView(APIView):
    def post(self, request, *args, **kwargs):
        comment_id = kwargs.get('comment_id')
        try:
            comment = NewsCommentModel.objects.get(_id=comment_id)
